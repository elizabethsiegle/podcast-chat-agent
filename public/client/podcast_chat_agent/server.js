...
class ErrorEvent extends Event {
  ...
  constructor(type, errorEventInitDict) {
    var _a15, _b;
    super(type), this.code = (_a15 = errorEventInitDict == null ? void 0 : errorEventInitDict.code) != null ? _a15 : void 0, this.message = (_b = errorEventInitDict == null ? void 0 : errorEventInitDict.message) != null ? _b : void 0;
  }
  /**
...
class SSEClientTransport {
  ...
  async send(message) {
    ...
    try {
      ...
      const headers = new Headers({ ...commonHeaders, ...(_a15 = this._requestInit) === null || _a15 === void 0 ? void 0 : _a15.headers });
...
function convertToCoreMessages(messages, options) {
  var _a17, _b;
  const tools2 = (_a17 = options == null ? void 0 : options.tools) != null ? _a17 : {};
  const coreMessages = [];
...
var MCPClientConnection = class {
  ...
  async init(code) {
    ...
    const [instructions, tools2, resources, prompts, resourceTemplates] = await Promise.all([
...
var DEFAULT_STATE = {};
var agentContext$1 = new AsyncLocalStorage();
var Agent = class extends Server {
  ...
  `;
    ...
    return this._tryCatch(() => {
      const { connection, request } = agentContext$1.getStore() || {};
      return agentContext$1.run(
...
var OpenAIChatLanguageModel = class {
  ...
  getArgs({
    ...
    switch (type) {
      case "regular": {
        const { tools: tools2, tool_choice, functions, function_call, toolWarnings } = prepareTools({
          mode,
          useLegacyFunctionCalling,
...
var OpenAIResponsesLanguageModel = class {
  ...
  getArgs({
    ...
    switch (type) {
      case "regular": {
        const { tools: tools2, tool_choice, toolWarnings } = prepareResponsesTools({
          mode,
          strict: isStrict
...
const scheduleTask = tool({
  description: "schedule a task to be executed at a later time. 'when' can be a date, a delay in seconds, or a cron pattern.",
  parameters: z.object({
  ...
  execute: async ({ type, when, payload }) => {
    ...
    try {
      agent.schedule(
        type === "scheduled" ? new Date(when) : type === "delayed" ? when : when,
        // cron
        "executeTask",
        payload
      );
...
class Chat extends AIChatAgent {
  constructor() {
    super(...arguments);
    this.initialState = { podcasts: [], lastUpdated: null };
  }
          ...
          const result = streamText({
            ...
            system: `
             You are a helpful podcast assistant that can generate podcasts and manage podcast content using Cloudflare Workers AI. You can:
              ...
              When users ask for podcasts, use the available tools to create and manage podcast content. The time is now: ${(/* @__PURE__ */ new Date()).toISOString()}.
              `,
  ...
  async generatePodcast(topic) {
    ...
    const response = await this.env.AI.run("@cf/meta/llama-4-scout-17b-16e-instruct", { messages });
  ...
  async listRecentPodcasts(limit = 10) {
    ...
    try {
      ...
      const podcastList = podcasts.map(
        (p) => `• ${p.topic} - ${p.url} (Generated: ${new Date(p.created_at).toLocaleString()})`
      ).join("\n");
...
const server = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/check-open-ai-key") {
      const hasKey = !!env2.OPENAI_API_KEY;
